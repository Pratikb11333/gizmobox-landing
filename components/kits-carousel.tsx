    ...
    <section id="kits-gallery" className="py-16 md:py-24 bg-background">
      <div className="container mx-auto px-4">
        <div className="mb-10 md:mb-12 text-center">
          <h2 className="text-3xl md:text-4xl font-semibold text-pretty">Gizmobox in Action</h2>
          <p className="mt-3 text-muted-foreground max-w-2xl mx-auto">
            Explore real builds you can create with the Gizmobox Complete Kit. Swipe or use the arrows to view example projects.
          </p>
        </div>
          ...
          <Carousel opts={{ loop: true, align: "start" }} className="w-full max-w-6xl mx-auto">
            <CarouselContent className="-ml-3">
              {slides.map((slide, idx) => (
                <CarouselItem key={`slide-${idx}`} className="pl-3 md:basis-1/2 lg:basis-1/3">
                  <div className="group rounded-lg border border-border bg-card overflow-hidden glow-cyan transition-shadow hover:glow-green">
                    <AspectRatio ratio={16 / 9} className="relative bg-muted/20">
                      ...
                      <div className="absolute bottom-3 left-3 right-3">
                        <p className="text-sm md:text-base font-medium">{slide.kit}</p>
                      </div>
                    </AspectRatio>
                  </div>
                </CarouselItem>
